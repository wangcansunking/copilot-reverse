...
// Multi-turn conversation continuity. The Anthropic/OpenAI wire is STATELESS — the client (Claude Code
// on `--resume`, or an interactive REPL) replays the ENTIRE prior conversation in `messages` on every
// turn. So the only thing that makes a follow-up turn "remember" the first is the proxy faithfully
// forwarding that replayed history to Copilot. These cases capture what actually reached the provider
// on turn 2 and assert the turn-1 exchange survived the translation layer — the deterministic core of
// what the real-CLI `--resume` fidelity case proves against live Copilot (see docker/cli-e2e.sh).
